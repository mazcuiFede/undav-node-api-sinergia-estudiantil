...
const signIn = (req, res) => {
  ...
  User.findOne({ documento: req.body.documento }, (err, user) => {
    ...
    if (!user)
      return res
        .status(404)
        .send({ msg: `no existe el usuario: ${req.body.documento}` })
    if (user.status === "inactivo")
      return res.status(404).send({
        msg: `Tu usuario está desactivado. Contactá a un administrador.`,
      })
    ...
    User.findOne({ documento: req.body.documento }, (err, user) => {
      ...
      return user.comparePassword(req.body.password, (err, isMatch) => {
        ...
        if (!isMatch)
          return res
            .status(404)
            .send({ msg: `Error de contraseña: ${req.body.documento}` })
    ...
    .then((response) => {
      User.findByIdAndUpdate({ _id: response }, status, (err, user) => {
        if (err)
          res
            .status(500)
            .send({ message: "hubo un error al actualizar el usuario" })
...
const getUsers = (req, res) => {
  ...
  User.find({}, (err, user) => {
    if (err)
      res
        .status(500)
        .send({ message: "hubo un error al obtener los administradores" })
